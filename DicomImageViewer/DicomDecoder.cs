...
namespace DicomImageViewer
{
    class DicomDecoder
    {
        ...
        public Slice ReadDicomFile(string fileName)
        {
            ...
            try
            {
                bool readResult = ReadFileInfo(slice);
                if (readResult && widthTagFound && heightTagFound && pixelDataTagFound)
                {
                    ReadPixels(slice);
                    if (dicmFound == true)
                        slice.typeofDicomFile = TypeOfDicomFile.Dicom3File;
                    else
        ...
        void AddInfo(int tag, string value, Slice slice)
        {
            ...
            if (info != null && str != ITEM)
            {
                ...
                slice.DicomInfo.Add(strPadded + "//" + strInfo);
            }
        }

        void AddInfo(int tag, int value, Slice slice)
        {
            AddInfo(tag, Convert.ToString(value), slice);
        }
        ...
        public bool ReadFileInfo(Slice slice)
        {
            ...
            while (decodingTags)
            {
                ...
                switch (tag)
                {
                    case (int)(TRANSFER_SYNTAX_UID):
                        ...
                        {
                            file.Close();
                            slice.typeofDicomFile = TypeOfDicomFile.DicomUnknownTransferSyntax;
                    ...
                    case (int)(SAMPLES_PER_PIXEL):
                        slice.SamplesPerPixel = GetShort();
                        AddInfo(tag, slice.SamplesPerPixel, slice);
                        break;
                    case (int)(PHOTOMETRIC_INTERPRETATION):
                        ...
                        AddInfo(tag, photoInterpretation, slice);
                        break;
                    case (int)(PLANAR_CONFIGURATION):
                        planarConfiguration = GetShort();
                        AddInfo(tag, planarConfiguration, slice);
                        break;
                    case (int)(ROWS):
                        slice.height = GetShort();
                        AddInfo(tag, slice.height, slice);
                        heightTagFound = true;
                        break;
                    case (int)(COLUMNS):
                        slice.width = GetShort();
                        AddInfo(tag, slice.width, slice);
                        widthTagFound = true;
                        break;
                    ...
                    case (int)(BITS_ALLOCATED):
                        ...
                        AddInfo(tag, slice.bitsAllocated, slice);
                        break;
                    case (int)(PIXEL_REPRESENTATION):
                        pixelRepresentation = GetShort();
                        AddInfo(tag, pixelRepresentation, slice);
                        break;
                    case (int)(WINDOW_CENTER):
                        ...
                        slice.windowCentre = Convert.ToDouble(center, new CultureInfo("en-US"));
                        AddInfo(tag, center, slice);
                        break;
                    case (int)(WINDOW_WIDTH):
                        String widthS = GetString(elementLength);
                        index = widthS.IndexOf('\\');
                        if (index != -1) widthS = widthS.Substring(index + 1);
                        slice.windowWidth = Convert.ToDouble(widthS, new CultureInfo("en-US"));
                        AddInfo(tag, widthS, slice);
                        break;
        ...
        void ReadPixels(Slice slice)
        {
            slice.pixels = new List<ushort>();

            if (slice.SamplesPerPixel == 1 && slice.bitsAllocated == 8)
            {                                
                int numPixels = slice.width * slice.height;
                ...
                for (int i = 0; i < numPixels; ++i)
                {
                    ...
                    slice.pixels.Add((byte)(pixelRepresentation == 1 ? pixVal : (pixVal - min8)));
                }
            }

            if (slice.SamplesPerPixel == 1 && slice.bitsAllocated == 16)
            {               
                if (pixels16Int != null)
                    pixels16Int.Clear();
                ...
                slice.signedImage = false;
                if (minPixVal < 0) slice.signedImage = true;
                ...
                foreach (int pixel in pixels16Int)
                {
                    ...
                    if (slice.signedImage)
                        slice.pixels.Add((ushort)(pixel - min16));
                    else
                        slice.pixels.Add((ushort)(pixel));
                }
            ...
            if (slice.SamplesPerPixel == 3 && slice.bitsAllocated == 8)
            {
                ...
                int numPixels = slice.width * slice.height;
                int numBytes = numPixels * slice.SamplesPerPixel;
